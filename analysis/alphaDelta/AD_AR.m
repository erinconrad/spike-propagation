...
This function exports data needed to prepare a GLARMA model in R in order
to determine whether there is a significant relationship between the
alpha-delta ratio and the proportion of spikes in the predominant cluster.

It also performs the ARIMA analysis testing whether the distance from the
nearest SOZ is related to the alpha delta ratio.
...
for whichPt = whichPts
    ...
    % popular cluster. X is the alpha delta ratio and ones for the constant
    % term
    Y = prop_pop;
    X = [mean_ad ones(size(mean_ad))];

...
for iTime = 1:size(seq,1)
    ...
    if iTime == 1
        imwrite(imind,cm,[resultsFolder,'plots/',filename],'gif', 'Loopcount',inf,'DelayTime',delay);
    else
        imwrite(imind,cm,[resultsFolder,'plots/',filename],'gif','WriteMode','append','DelayTime',delay);
    end

...
ictalSeq = allSeq(:,any(firstSpikes >= szTimes(:,1) & ...
    firstSpikes <= szTimes(:,2),1));
%}
nonIctalSeq = allSeq(:,~any(repmat(firstSpikes,size(szTimes,1),1) >= ...
    repmat(szTimes(:,1),1,size(firstSpikes,2)) & ...
    repmat(firstSpikes,size(szTimes,1),1) <= ...
    repmat(szTimes(:,2),1,size(firstSpikes,2)),1));
ictalSeq = allSeq(:,any(repmat(firstSpikes,size(szTimes,1),1) >= ...
    repmat(szTimes(:,1),1,size(firstSpikes,2)) & ...
    repmat(firstSpikes,size(szTimes,1),1) <= ...
    repmat(szTimes(:,2),1,size(firstSpikes,2)),1));

...
function [gdf,noise,removed] = fspk7(eeg,tmul,absthresh,n_chans,...
    srate,window,electrodes,name)
...
% Remove certain electrodes
elecsToIgnore = icChsToIgnore(name);
chIds = zeros(size(elecsToIgnore));
for ich = 1:length(elecsToIgnore)
    [Lia,chIds(ich)] = ismember(elecsToIgnore{ich},electrodes.unignoredChs);
    if Lia == 0
        fprintf('Warning, could not find channel %s in the list of unignored channels\n',...
            elecsToIgnore{ich});
        error('look');
    end
end
...
noise = zeros(num_segs-1,max(chan));

%{
comprat = @(x,ch1) mean(abs(eegfilt(alldata(:,ch1),x,'lp',srate)).^2);
for i = 1:n_chans
   fprintf('Low freq amplitude for channel %d is %1.1e\n',i, comprat(0.3,i));
...
for dd = chan
    ...
    for tt = 1:num_segs
        ...
        %% Skip if just high amplitude noise
        if 1 == 0
       ha_lf = mean(abs(eegfilt(data,1,'lp',srate)).^2);
       if ha_lf > 1e4
           fprintf('Skipping channel %d due to high amplitude low frequency component\n',dd);
           continue; 
       end
        end

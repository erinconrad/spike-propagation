...
[gdf,extraoutput] = getSpikesSimple(pt,whichPt,times,7,thresh,0);
...
for i = 1:length(spike_chs)
    ...
    %scatter(s_time-times(1),sp_vals(i)+toAdd,100,'b','filled');
    y_locs(i) = toAdd;
    if i ~= length(spike_chs)
       toAdd = toAdd + min(values(:,ch)) - max(values(:,spike_chs(i+1)))-500; 
    end
